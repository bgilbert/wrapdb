...
def do_list(args: Namespace) -> None:
    ...
    for name in cur_vers:
        if names and name not in names:
            ...
            continue
        if (args.official or args.port) and args.port != (name in ports):
            # user wants only official or only ports and this wrap is the
            # opposite
            continue
        if name in upstream_vers:
            if args.untracked:
                # user wants untracked wraps and this one is tracked
                continue
            if (
                (args.current or args.update) and
                args.current != (cur_vers[name] == upstream_vers[name])
            ):
                # user wants only current or only updated and this wrap is
                # the opposite
                continue
        elif args.current or args.update:
            # user wants tracked wraps and this one is untracked
            continue
        wraps.append(name)
    ...
    if args.github:
        ...
        json.dump(
            {
                "include": [
                    {'wrap': name, 'new-version': upstream_vers.get(name)}
                    for name in wraps
                ]
            },
            sys.stdout,
        ...
        for name in wraps:
            line = f'{name:25} {cur_vers[name]:>15}'
            if cur_vers[name] != upstream_vers.get(name):
                line += f'  => {upstream_vers.get(name, "???"):>15}'
            print(line)
